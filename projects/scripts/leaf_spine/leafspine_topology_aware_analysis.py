...
try:
    from projects.leaf_spine.topology_aware import LeafSpineTopo, ElasticTreeOptimizer
except ImportError as e:
    print(f"ERROR: Could not import from topology_aware.py")
...
def generate_traffic_matrix(topo, active_ratio, pattern_type, seed):
    ...
    for i, host in enumerate(all_hosts):
        leaf_idx = i // topo.hosts_per_leaf
        if leaf_idx < len(topo.LeafSwitchList):
            host_to_leaf[host] = topo.LeafSwitchList[leaf_idx]
...
def run_single_topology_aware_optimization(active_ratio, traffic_pattern, seed):
    ...
    active_leaves = sum(1 for s in topo.LeafSwitchList if s in required_switches)
    active_spines = sum(1 for s in topo.SpineSwitchList if s in required_switches)
    
    return power_consumption, num_active_switches, active_leaves, active_spines
...
def plot1_power_vs_load():
    ...
    for i, load in enumerate(loads):
        ...
        p_near, _, _, _ = run_single_topology_aware_optimization(load, 'near', seed)
        power_near.append(p_near)
        
        p_mid, _, _, _ = run_single_topology_aware_optimization(load, 'balanced', seed + 1)
        power_mid.append(p_mid)
        
        p_far, _, _, _ = run_single_topology_aware_optimization(load, 'far', seed + 2)
    ...
    plt.title('Leaf-Spine Topology-Aware: Power vs Load\n', 
    ...
    plt.tight_layout()
    plt.savefig('topology_aware_plot1_power_vs_load.png', dpi=300, bbox_inches='tight')
...
def plot2_random_traffic_scatter():
    ...
    for i in range(num_samples):
        ...
        power, _, _, spine_count = run_single_topology_aware_optimization(load, pattern, seed=i*100)
    ...
    plt.tight_layout()
    plt.savefig('topology_aware_plot2_random_traffic_scatter.png', dpi=300, bbox_inches='tight')
...
def plot3_diurnal_variation():
    ...
    for i, load in enumerate(load_variation):
        ...
        p, _, _, _ = run_single_topology_aware_optimization(load, 'balanced', seed=int(i*100))
    ...
    plt.tight_layout()
    plt.savefig('topology_aware_plot3_diurnal_variation.png', dpi=300, bbox_inches='tight')
