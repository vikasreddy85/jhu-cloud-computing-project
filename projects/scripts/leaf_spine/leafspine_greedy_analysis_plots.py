...
try:
    from projects.leaf_spine.greedy import LeafSpineTopo, GreedyBinPackingOptimizer
except ImportError as e:
    print(f"ERROR: Could not import from leafspine_greedy_binpacking.py")
...
def generate_traffic_pattern(topo, active_ratio, pattern_type, seed):
    ...
    for i, host in enumerate(all_hosts):
        ...
        if leaf_idx < len(topo.LeafList):
            host_to_leaf[host] = topo.LeafList[leaf_idx]
    
    for host in all_hosts:
        if host in active_hosts:
            host_leaf = host_to_leaf.get(host)
            
            if pattern_type == 'near':
                same_leaf_count = sum(1 for h in active_hosts 
                                     if host_to_leaf.get(h) == host_leaf)
                if same_leaf_count > 1:
...
def run_single_greedy_optimization(active_ratio, traffic_pattern, seed):
    ...
    required_switches, leaf_utilization = optimizer.greedy_bin_packing(host_traffic)
    
    power_consumption = len(required_switches) * POWER_PER_SWITCH
    num_active_switches = len(required_switches)
    
    active_spines = sum(1 for s in topo.SpineList if s in required_switches)
    
    return power_consumption, num_active_switches, leaf_utilization, active_spines
...
def plot1_power_vs_load():
    ...
    plt.title('Leaf-Spine Greedy Bin-Packing: Power vs Load\n', 
    ...
    plt.tight_layout()
    plt.savefig('greedy_plot1_power_vs_load.png', dpi=300, bbox_inches='tight')
...
def plot2_random_traffic_scatter():
    ...
    for i in range(num_samples):
        ...
        power, _, _, spine_count = run_single_greedy_optimization(load, pattern, seed=i*100)
    ...
    plt.tight_layout()
    plt.savefig('greedy_plot2_random_traffic_scatter.png', dpi=300, bbox_inches='tight')
...
def plot3_diurnal_variation():
    ...
    plt.tight_layout()
    plt.savefig('greedy_plot3_diurnal_variation.png', dpi=300, bbox_inches='tight')
