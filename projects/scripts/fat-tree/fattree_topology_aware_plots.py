...
try:
    from projects.fattree.topology_aware import FatTree, ElasticTreeOptimizer
except ImportError as e:
    print(f"ERROR: Could not import from topology_aware_fixed.py")
...
def generate_traffic_matrix(topo, active_ratio, pattern_type, seed):
    ...
    for i, host in enumerate(all_hosts):
        ...
        host_to_pod[host] = pod_idx
    
    for src in active_hosts:
        src_pod = host_to_pod.get(src, 0)
        
        for dst in active_hosts:
            if src == dst:
                continue
            
            dst_pod = host_to_pod.get(dst, 0)
            
            if pattern_type == 'near':
                if src_pod == dst_pod:
                    traffic_matrix[src][dst] = random.uniform(0.2, 0.5)
                else:
...
def run_single_topology_aware_optimization(active_ratio, traffic_pattern, seed):
    ...
    active_core = sum(1 for s in topo.CoreSwitchList if s in required_switches)
    active_agg = sum(1 for s in topo.AggSwitchList if s in required_switches)
    active_edge = sum(1 for s in topo.EdgeSwitchList if s in required_switches)
    
    return power_consumption, num_active_switches, active_core, active_agg, active_edge
...
def plot1_power_vs_load():
    ...
    for i, load in enumerate(loads):
        ...
        p_near, _, _, _, _ = run_single_topology_aware_optimization(load, 'near', seed)
        power_near.append(p_near)
        
        p_mid, _, _, _, _ = run_single_topology_aware_optimization(load, 'balanced', seed + 1)
        power_mid.append(p_mid)
        
        p_far, _, _, _, _ = run_single_topology_aware_optimization(load, 'far', seed + 2)
    ...
    plt.title(f'Fat-Tree Topology-Aware: Power vs Load\n', 
    ...
    plt.tight_layout()
    plt.savefig('fattree_topology_aware_plot1_power_vs_load.png', dpi=300, bbox_inches='tight')
    print("  Saved: fattree_topology_aware_plot1_power_vs_load.png")
...
def plot2_random_traffic_scatter():
    ...
    for i in range(num_samples):
        ...
        power, _, core_count, _, _ = run_single_topology_aware_optimization(load, pattern, seed=i*100)
    ...
    plt.xlabel('Network Load (%)', fontsize=13, fontweight='bold')
    plt.ylabel('Power Consumption (W)', fontsize=13, fontweight='bold')
    plt.title(f'Fat-Tree Random Traffic Scenarios: Load vs Power\n', 
              fontsize=14, fontweight='bold')
    plt.legend(loc='upper left', fontsize=11)
    plt.grid(True, alpha=0.3, linestyle='--')
    plt.xlim(5, 105)
    ...
    stats_text = f'Overall Average:\nPower: {avg_power:.1f}W\nSavings: {avg_savings:.1f}%\nAvg Cores: {avg_cores:.1f}\nSamples: {num_samples}'
    plt.text(0.98, 0.05, stats_text, transform=plt.gca().transAxes,
             fontsize=10, verticalalignment='bottom', horizontalalignment='right',
             bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig('fattree_topology_aware_plot2_random_traffic_scatter.png', dpi=300, bbox_inches='tight')
...
def plot3_diurnal_variation():
    ...
    for i, load in enumerate(load_variation):
        ...
        p, _, _, _, _ = run_single_topology_aware_optimization(load, 'balanced', seed=int(i*100))
    ...
    plt.ylabel('Power Consumption (W)', fontsize=13, fontweight='bold')
    plt.title(f'Fat-Tree Diurnal Variation: 24-Hour Power Consumption\n', 
              fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig('fattree_topology_aware_plot3_diurnal_variation.png', dpi=300, bbox_inches='tight')
