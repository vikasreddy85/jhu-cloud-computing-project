...
try:
    from projects.fattree.greedy import FatTree, GreedyBinPackingOptimizer
except ImportError as e:
    print(f"ERROR: Could not import from greedy_binpacking.py")
...
def generate_traffic_pattern(topo, active_ratio, pattern_type, seed):
    ...
    for host in all_hosts:
        if host in active_hosts:
            host_edge = host_to_edge.get(host)
            host_pod = edge_to_pod.get(host_edge)
            
            if pattern_type == 'near':
                same_pod_count = sum(1 for h in active_hosts 
                                    if host_to_edge.get(h) and 
                                    edge_to_pod.get(host_to_edge.get(h)) == host_pod)
                if same_pod_count > 1:
...
def run_single_greedy_optimization(active_ratio, traffic_pattern, seed):
    ...
    host_traffic = generate_traffic_pattern(topo, active_ratio, traffic_pattern, seed)
    
    required_switches, edge_utilization = optimizer.greedy_bin_packing(host_traffic)
    ...
    return (power_consumption, num_active_switches, edge_utilization, 
...
def plot1_power_vs_load():
    ...
    for i, load in enumerate(loads):
        ...
        p_near, _, _, _, _, _ = run_single_greedy_optimization(load, 'near', seed)
        power_near.append(p_near)
        
        p_mid, _, _, _, _, _ = run_single_greedy_optimization(load, 'balanced', seed + 1)
        power_mid.append(p_mid)
        
        p_far, _, _, _, _, _ = run_single_greedy_optimization(load, 'far', seed + 2)
    ...
    plt.title('Fat-Tree Greedy Bin-Packing: Power vs Load\n', 
    ...
    plt.tight_layout()
    plt.savefig('fattree_greedy_plot1_power_vs_load.png', dpi=300, bbox_inches='tight')
...
def plot2_random_traffic_scatter():
    ...
    for i in range(num_samples):
        ...
        power, _, _, core_count, _, _ = run_single_greedy_optimization(load, pattern, seed=i*100)
    ...
    plt.tight_layout()
    plt.savefig('fattree_greedy_plot2_random_traffic_scatter.png', dpi=300, bbox_inches='tight')
...
def plot3_diurnal_variation():
    ...
    for i, load in enumerate(load_variation):
        ...
        p, _, _, _, _, _ = run_single_greedy_optimization(load, 'balanced', seed=int(i*100))
    ...
    plt.tight_layout()
    plt.savefig('fattree_greedy_plot3_diurnal_variation.png', dpi=300, bbox_inches='tight')
