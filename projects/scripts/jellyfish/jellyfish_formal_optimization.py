...
try:
    from projects.jellyfish.formal import JellyfishTopo, JellyfishFormalOptimizer
except ImportError as e:
    print(f"ERROR: Could not import from jellyfish_formal_optimization.py")
...
NUM_PORTS = 8
NUM_SWITCH_PORTS = 4
NUM_HOST_PORTS = NUM_PORTS - NUM_SWITCH_PORTS
TOTAL_HOSTS = NUM_SWITCHES * NUM_HOST_PORTS
POWER_PER_SWITCH = 10
...
def generate_traffic_matrix(topo, active_ratio, pattern_type, seed):
    ...
    for switch_idx, switch in enumerate(topo.SwitchList):
        start_host = switch_idx * topo.num_host_ports
        end_host = min(start_host + topo.num_host_ports, len(topo.HostList))
    ...
    for src in active_hosts:
        ...
        same_switch_hosts = [h for h in switch_to_hosts[src_switch] 
                            if h in active_hosts and h != src]
        other_switch_hosts = [h for h in active_hosts 
                             if h != src and h not in same_switch_hosts]
...
def run_single_formal_optimization(active_ratio, traffic_pattern, seed):
    ...
    optimizer.traffic_matrix = generate_traffic_matrix(topo, active_ratio, traffic_pattern, seed + 1000)    
    ...
    for switch in required_switches:
        active_neighbors = sum(1 for n in optimizer.switch_neighbors[switch] 
                              if n in required_switches)
        active_degree += active_neighbors
    
    avg_degree = active_degree / len(required_switches) if required_switches else 0    
    is_connected = False
    if required_switches:
        component = optimizer.get_connected_component(required_switches)
        is_connected = len(component) == len(required_switches)
    ...
    return power_consumption, num_active_switches, num_active_links, graph_props
...
def plot1_power_vs_load():
    ...
    plt.title('Jellyfish Formal Optimization: Power vs Load\n', 
    ...
    plt.tight_layout()
    plt.savefig('jellyfish_plot1_power_vs_load.png', dpi=300, bbox_inches='tight')
...
def plot2_random_traffic_scatter():
    ...
    for i in range(num_samples):
        ...
        power, _, _, graph_props = run_single_formal_optimization(load, pattern, seed=i*100)
        
        loads.append(load * 100)
        powers.append(power)
        patterns.append(pattern)
        avg_degrees.append(graph_props['avg_degree'])
    ...
    plt.tight_layout()
    plt.savefig('jellyfish_plot2_random_traffic_scatter.png', dpi=300, bbox_inches='tight')
...
def plot3_diurnal_variation():
    ...
    plt.tight_layout()
    plt.savefig('jellyfish_plot3_diurnal_variation.png', dpi=300, bbox_inches='tight')
